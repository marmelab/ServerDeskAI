-- Fix: PostgREST cannot resolve empty JS array [] to uuid[].
-- Adding a DEFAULT allows the frontend to omit p_company_ids when empty.

DROP FUNCTION IF EXISTS public.create_invite(text, app_role, uuid[]);
